...
function AmountInWords({ invAmount }) {
  var op = "";
  const [amntInWords, setamntInWords] = useState("");

  useEffect(async () => {
    await RsPaise(Math.round(parseFloat(invAmount) * 100) / 100);
    setamntInWords(op);
  }, []);
  ...
  function RsPaise(n) {
    ...
    if (nums.length == 2) {
      ...
      if (invAmount > 999999999.99) {
        op = "Oops!!! The amount is too big to convert";
      }
      if (isNaN(invAmount) == true) {
